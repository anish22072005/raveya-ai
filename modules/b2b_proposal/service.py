...
async def generate_proposal(
    ...
    logger.info("Proposal %s created for '%s' — budget ₹%.2f", proposal_id, request.company_name, request.budget)
...
def _validate_and_fix_budget(ai_data: dict, max_budget: float) -> None:
    ...
    if allocated > max_budget:
        logger.warning("AI over-allocated ₹%.2f vs budget ₹%.2f — capping.", allocated, max_budget)
